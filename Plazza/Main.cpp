...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            throw std::invalid_argument("Invalid multiplier format");
        }

        cooksPerKitchen = std::stoi(argv[2], &pos);
        if (pos != std::string(argv[2]).length())
        {
            throw std::invalid_argument("Invalid cooks_per_kitchen format");
        }

        ingredientRestockTimeMs = std::stoll(argv[3], &pos);
        if (pos != std::string(argv[3]).length())
        {
            throw std::invalid_argument("Invalid restock_time_ms format");
        }

        if (cookingTimeMultiplier <= 0)
        {
            std::cerr << "Error: Cooking time multiplier must be positive." << std::endl;
            return 84;
        }
        if (cooksPerKitchen <= 0)
        {
            std::cerr << "Error: Number of cooks per kitchen must be positive." << std::endl;
            return 84;
        }
        if (ingredientRestockTimeMs <= 0)
        {
            std::cerr << "Error: Ingredient restock time must be positive." << std::endl;
            return 84;
        }
    }
